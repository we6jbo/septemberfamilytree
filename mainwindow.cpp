...
0|Jeremiah O'Neal|XX/XX/XXXX|I am the author of this program and this is my family tree. I don't want any information about myself in here because I intend to publish it in the Snap Store.
...
1|Mom||Private individual. While everyone else's information is public, this person's information is private.
2|Grandma||Private individual. While everyone else's information is public, this person's information is private.
...
2|Grandpa||Private individual. While everyone else's information is public, this person's information is private.
      ...
      ui(new Ui::MainWindow)
{
    ...
    for (const QString &line : QString::fromUtf8(treeData).split('\n')) {
        ...
        QList<QString> parts;
        int from = 0;
        for (int i = 0; i < 3 && from >= 0; ++i) {
            int bar = line.indexOf('|', from);
            if (i < 2) {
                if (bar < 0) { parts.clear(); break; }
                parts.append(line.mid(from, bar - from));
                from = bar + 1;
            } else {
                parts.append(line.mid(from));
                from = -1;
            }
        }
        if (parts.size() < 3)
        ...
        int depth = parts[0].toInt();
        QString name = parts[1];
        QString tg = parts[2];
        QString note = parts.size() > 3 ? parts[3] : QString();
